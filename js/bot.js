...
function getBotResponse(input) {
 ...
 return 'Class ' + toBangla(cls) + ' এর বিষয়গুলো দেখাচ্ছি... <i class="fas fa-book"></i>';
 }
 }
 ...
 navigate('class.html?class=6');
 return 'বাংলা বিষয় খুলছি... <i class="fas fa-book-open"></i>';
 }
 ...
 return '<i class="fas fa-fire"></i> তোমার স্ট্রিক: ' + toBangla(streak) + ' দিন\n' +
 ...
 const results = JSON.parse(localStorage.getItem('cpb_results') || '[]');
 if (results.length === 0) {
 return 'এখনো কোনো পরীক্ষা দাওনি। "পরীক্ষা" লিখে শুরু করো! <i class="fas fa-dumbbell"></i>';
 }
 let best = 0;
 results.forEach(r => {
 ...
 return '<i class="fas fa-chart-bar"></i> পরীক্ষা: ' + toBangla(results.length) + 'টি\n' +
 '<i class="fas fa-trophy"></i> সেরা স্কোর: ' + toBangla(Math.round(best)) + '%';
 }
 ...
 return 'স্বাগতম! পড়াশোনায় এগিয়ে যাও! <i class="fas fa-book"></i>';
 }

 if (matches(msg, ['বিদায়', 'bye', 'গুডবাই', 'আল্লাহ হাফেজ'])) {
 return 'আল্লাহ হাফেজ! আবার এসো <i class="fas fa-heart"></i>';
 }

 // Help / Commands
 if (matches(msg, ['সাহায্য', 'help', 'কমান্ড', 'commands', 'কী করতে পারো'])) {
 return 'আমি যা করতে পারি:\n\n' +
 '<i class="fas fa-pen"></i> "পরীক্ষা" — পরীক্ষা শুরু\n' +
 '<i class="fas fa-book"></i> "সিলেবাস" — সিলেবাস দেখাও\n' +
 '<i class="fas fa-chart-bar"></i> "অগ্রগতি" — প্রোগ্রেস দেখাও\n' +
 '<i class="fas fa-book-open"></i> "নোট" — পড়ার নোট\n' +
 '<i class="fas fa-user"></i> "প্রোফাইল" — আমার তথ্য\n' +
 '<i class="fas fa-fire"></i> "স্ট্রিক" — কতদিন টানা পড়ছি\n' +
 '<i class="fas fa-trophy"></i> "স্কোর" — সেরা স্কোর\n' +
 ...
 return 'চিন্তা করো না! <i class="fas fa-seedling"></i>\n' +
 'ছোট করে শুরু করো — মাত্র ৫ মিনিট।\n' +
 'একবার শুরু করলে আর থামতে মন চাইবে না! <i class="fas fa-dumbbell"></i>';
 }

 if (matches(msg, ['কঠিন লাগছে', 'বুঝতে পারছি না', 'পারব না'])) {
 return 'কঠিন লাগলে চ্যাপ্টার ভেঙে ভেঙে পড়ো।\n' +
 'নোট পড়ো MCQ করো আবার পড়ো।\n' +
 'তুমি পারবে, ইনশাআল্লাহ! <i class="fas fa-star"></i>';
 }

 if (matches(msg, ['মোটিভেশন', 'মোটিভেট', 'অনুপ্রেরণা'])) {
 return 'সাফল্য একদিনে আসে না।\n' +
 'প্রতিদিন একটু একটু করে এগোলেই একদিন অনেক দূর পৌঁছে যাবে! <i class="fas fa-rocket"></i>';
 }
 ...
 if (h > 12) h -= 12;
 if (h === 0) h = 12;
 return '<i class="fas fa-clock"></i> এখন ' + toBangla(h) + ':' + toBangla(m.toString().padStart(2, '0')) + ' ' + ampm;
 }
...
function initBot() {
 ...
 '<button class="bot-quick-btn" data-msg="পরীক্ষা"><i class="fas fa-pen"></i> পরীক্ষা</button>' +
 '<button class="bot-quick-btn" data-msg="সিলেবাস"><i class="fas fa-book"></i> সিলেবাস</button>' +
 '<button class="bot-quick-btn" data-msg="স্ট্রিক"><i class="fas fa-fire"></i> স্ট্রিক</button>' +
