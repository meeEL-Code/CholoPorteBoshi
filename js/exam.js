...
function displayResult(correct, total, wrong, timeTaken) {
    ...
    if (percent >= 80) {
        svgIcon = SVG.trophy; title = 'অসাধারণ!'; message = 'তুমি দারুণ করেছ! এভাবেই এগিয়ে যাও।'; colorClass = 'icon-gold';
    } else if (percent >= 60) {
        svgIcon = SVG.thumbsUp; title = 'ভালো করেছ!'; message = 'আরেকটু চেষ্টা করলে আরও ভালো হবে।'; colorClass = 'icon-green';
    } else if (percent >= 40) {
        svgIcon = SVG.book; title = 'চর্চা দরকার'; message = 'আরও পড়াশোনা করো, আবার পরীক্ষা দাও।'; colorClass = 'icon-blue';
    } else {
        svgIcon = SVG.muscle; title = 'হাল ছাড়বে না!'; message = 'আবার পড়ো এবং আবার চেষ্টা করো।'; colorClass = 'icon-red';
    }
...
function renderReview(reviewData) {
    ...
    reviewList.innerHTML = reviewData.map((r, i) => {
        ...
        if (r.userAnswer === null) {
            userAnsHtml = '<span class="not-answered">' + SVG.dash + ' উত্তর দাওনি</span>';
        } else if (r.isCorrect) {
            userAnsHtml = '<span class="correct-text">' + SVG.check + ' সঠিক</span>';
        } else {
            userAnsHtml = '<span class="wrong-text">' + SVG.cross + ' ভুল</span>';
        }
